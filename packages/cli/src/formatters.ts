...
/**
 * Format issues as plain text with colors
 */
export function formatText(issuesByFile: Map<string, SSRIssue[]>, options: FormatOptions = {}): string {
  ...
  if (stats.total === 0) {
    return chalk.green('✓ No SSR compatibility issues found!\n');
  }
  ...
  output += chalk.dim(`   ${stats.errors} error(s), ${stats.warnings} warning(s)\n\n`);
  ...
  for (const [file, issues] of issuesByFile) {
    ...
    for (const issue of issues) {
      ...
      output += `  ${severityIcon} ${location} ${chalk.bold(issue.api)} - ${issue.message}\n`;

      if (options.verbose) {
        output += chalk.dim(`     ${issue.code}\n`);
        if (issue.suggestion) {
          output += chalk.blue(`     💡 ${issue.suggestion}\n`);
        }
      }
    }
  }
  ...
  // Tips
  output += chalk.dim('\n💡 Tips:\n');
  output += chalk.dim('   - Use "typeof window !== \'undefined\'" to check for browser environment\n');
  output += chalk.dim('   - Move browser API calls to useEffect hooks\n');
  output += chalk.dim('   - Add { ssr: false } to next/dynamic imports\n\n');
